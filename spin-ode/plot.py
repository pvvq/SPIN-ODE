...
def plot_series(
    ...
    if t is None:
        t = range(y.shape[1])
    if tt is None and yy is not None:
        tt = range(yy.shape[1])
    ...
    for i in range(n_species):
        ...
        if yy is not None:
            ax.scatter(
                tt, yy[i], facecolors="none", edgecolors="black", s=10, linewidth=0.3
            )
...
def plot_err_k(true_k, k_list, label_list):
    num_react = k_list[0].shape[0]
    ...
    ax.set_ylabel("SMAPE error of Rate Coefficient")
    # ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1))
    return fig


def plot_k(k_list, label_list):
    num_react = k_list[0].shape[0]
    marker_list = ["x", "+", "_"]
    fig, ax = plt.subplots(layout="constrained")
    for i in range(len(k_list)):
        ax.scatter(
            range(num_react), k_list[i], label=label_list[i], marker=marker_list[i]
        )
    ax.set_yscale("log")
    ax.legend()
    ax.set_xlabel("No. of reactions")
